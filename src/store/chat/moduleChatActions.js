...
export default {
    ...
    fetchChatContacts ({ getters, commit }) {
        return new Promise((resolve, reject) => {
            ...
            .then((response) => {
                ...
                let data = []
                response.data.forEach(function(thread,index) {
                    data[thread.id] = thread.conversations
                });
                commit('UPDATE_CHATS', {chats:data,thread_id:0})
    ...
    fetchChats ({ commit }, thread) {
        return new Promise((resolve, reject) => {
            ...
            .then((response) => {
                let data = []
                data[thread.id] = response.data

                commit('UPDATE_CHATS', {chats:data,thread_id:thread.id})
                resolve(response)
            })
            .catch((error) => { reject(error) })
        })
    ...
    markSeenAllMessages ({ getters, commit }, thread) {
        return new Promise((resolve, reject) => {
            axios.post('/api/chat/message/mark-seen', {thread})
            .then((response) => {
                commit('MARK_SEEN_ALL_MESSAGES', {
                    id: thread,
                    chatData: getters.chatDataOfUser(thread.id)
                })
                resolve(response)
            })
            .catch((error) => { reject(error) })
